...
const getDeviceRecency = (device) => { var _a, _b, _c; return (_c = (_b = (_a = device.lastSeenAt) !== null && _a !== void 0 ? _a : device.lastRegisteredAt) !== null && _b !== void 0 ? _b : device.updatedAt) !== null && _c !== void 0 ? _c : 0; };
const getPhysicalDeviceKey = (device) => {
    var _a, _b, _c, _d;
    if (device.isLegacy || !device.isPhysicalDevice) {
        return null;
    }
    const deviceName = (_a = normalizeString(device.deviceName)) === null || _a === void 0 ? void 0 : _a.toLowerCase();
    const modelName = (_b = normalizeString(device.modelName)) === null || _b === void 0 ? void 0 : _b.toLowerCase();
    if (!deviceName || !modelName) {
        return null;
    }
    return [
        device.platform,
        (_d = (_c = normalizeString(device.projectId)) === null || _c === void 0 ? void 0 : _c.toLowerCase()) !== null && _d !== void 0 ? _d : "unknown-project",
        deviceName,
        modelName,
    ].join(":");
};
const dedupeDevicesForDelivery = (devices) => {
...
const retireSupersededDevices = async (userId, currentDeviceId) => {
    ...
    const supersededDevices = devices.filter((device) => {
        ...
        const sharesToken = Boolean(currentToken &&
            normalizeString(device.expoPushToken) === currentToken);
        const sharesPhysicalDevice = Boolean(currentPhysicalKey &&
    ...
    await Promise.all(supersededDevices.map((device) => (0, firestore_1.getFirestore)().doc(device.path).set({
        expoPushToken: null,
